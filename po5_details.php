...
<body>
    ...
    <div class="dropdown">
      ...
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal1">5.1.1 Identify modern engineering tools such as computer-aided drafting, modeling and analysis; techniques and resources for engineering activities</a>
        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal1">5.1.2 Create/adapt/modify/extend tools and techniques to solve engineering problems</a>
      </div>
    </div>
    ...
    <div class="dropdown">
      ...
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal1">5.2.1 Identify the strengths and limitations of tools for (i) acquiring information, (ii)modeling and simulating, (iii) monitoring system performance, and (iv) creating engineering designs.</a>
        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal1">5.2.2 Demonstrate proficiency in using discipline-specific tools</a>
      </div>
    </div>

    <div class="dropdown">
      <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        5.3: Demonstrate an ability to evaluate the suitability and limitations of tools used to slve an engineering problem
      </button>
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal1">5.3.1 Discuss limitations and validate tools, techniques and resources </a>
        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal1">5.3.2 Verify the credibility of results from tool use with reference to the accuracy and limitations, and the assumptions inherent in their use.</a>
      </div>
    </div>
  </div>
...
if (isset($_POST['map_data'])) {
    ...
    for ($i = 1; $i <= $total_count; $i++) {
        ...
        if (!empty($_POST[$input_id])) {
            $input_value = round($_POST[$input_id] * 3 / 6);
        } else {
            $input_value = "";
        }
        ...
        if ($check_result && mysqli_num_rows($check_result) > 0) {
            ...
            $update_query = "UPDATE mapping SET PO5 = ? WHERE co_id = ? AND year = ? AND subject_id = ?";
            ...
            $insert_query = "INSERT INTO mapping (PO5, co_id, year, subject_id) VALUES (?, ?, ?, ?)";
